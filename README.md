...
- [Отключение хуков на уровне каталога](#отключение-хуков-на-уровне-каталога)
...
## Отключение хуков на уровне каталога

Глобальные `PreToolUse`-хуки в `~/.claude/settings.json` действуют во всех сессиях. Чтобы отключить конкретный хук только в одном каталоге, используется секция `env` в `.claude-dir-settings.yaml`:

```yaml
env:
  CLAUDE_HOOK_GIT_COMMIT_PUSH_DISABLED: "1"
```

Wrapper экспортирует эту переменную перед запуском `claude`, она наследуется дочерними процессами хуков. Сам hook-скрипт проверяет переменную в начале блока и при значении `1` пропускает свою логику.

Конвенция имени: `CLAUDE_HOOK_<NAME>_DISABLED=1`, где `<NAME>` -- идентификатор конкретного хука в верхнем регистре.

| Переменная                              | Что отключает                                                |
|-----------------------------------------|--------------------------------------------------------------|
| `CLAUDE_HOOK_GIT_COMMIT_PUSH_DISABLED`  | Подтверждение `permissionDecision: ask` на `git commit`/`git push` |

Особенности:
- Работает только при запуске через wrapper (env выставляется в `claude-dir-settings-env.sh`)
- Действует на всю сессию -- env фиксируется при запуске `claude`
- Не отменяет blocking-проверки внутри того же хука (например, запрет `Co-Authored-By`), отключение явно реализовано только для конкретных веток case

Поддержка переменной должна быть прописана в самом hook-скрипте. Список поддерживаемых переменных ведётся в таблице выше.
